...
describe("Brush focus-context behaviour", () => {
    ...
    it("commits resize when currentItem is clamped at edge but mouse is outside range", () => {
        const { brush, buildMoreProps, onBrush } = createHarness();
        ...
        act(() => {
            (brush as any).handleZoomStart({} as React.MouseEvent, buildMoreProps(7));
        });

        act(() => {
            (brush as any).handleDrawSquare({ buttons: 1 } as unknown as React.MouseEvent, {
                ...buildMoreProps(9),
                mouseXY: [120, 50],
            });
        });
        ...
        act(() => {
            (brush as any).handleDrawSquare({ buttons: 0 } as unknown as React.MouseEvent, buildMoreProps(6));
        });
